...
## DELETE OS ARQUIVOS ANTIGOS ANTES DE GERAR UM NOVO
    ...
    for (i in unique(group_pre)){
      ...
      ## Modelos bin?rios
      if (write.bin==T){
        cat(paste("Salvando modelos binÃ¡rios...",sp,i,'\n'))
        ...
        if(Domain==T){
          ...
          plot(do_bin,main=paste("Domain - Binario ",i))
          dev.off()
        ...
        if(maxent==T){
          ...
          plot(mx_bin,main=paste("Maxent - Binario ",i))
          dev.off()
        ...
        if(GLM==T){
          ...
          plot(glm_bin,main=paste("GLM - BinÃ¡rio ",i))
          dev.off()
        ...
        if(RF==T){
          ...
          plot(rf1_bin,main=paste("RF - BinÃ¡rio ",i))
          dev.off()
        ...
        if(SVM==T){
          ...
          plot(svm_bin,main=paste("SVM - BinÃ¡rio ",i))
          dev.off()
        ...
        if(Mahal==T && condicao_Mahal==TRUE){
          ...
          plot(ma_bin,main=paste("Mahalanobis - BinÃ¡rio ",i))
          dev.off()
      ...
      if(Domain==T){
        print(paste(sp,sp,i,"Domain",round(edo@auc,3),round(doTSS,3),round(tdo,3),round(threshold(edo)$kappa,3),round(threshold(edo)$equal_sens_spec,3),round(threshold(edo)$no_omission,3),round(threshold(edo)$prevalence,3),round(threshold(edo)$sensitivity,3),sep=","))}
      if(maxent==T){
 ...
 #     cat(paste("Salvando arquivos de validação para todas as espécies...",sp,i,'\n'))
  ...
  #      print(paste(sp,sp,i,"Mahal",round(ema@auc,3),round(maTSS,3),round(tma,3),round(threshold(ema)$kappa,3),round(threshold(ema)$equal_sens_spec,3),round(threshold(ema)$no_omission,3),round(threshold(ema)$prevalence,3),round(threshold(ema)$sensitivity,3),sep=","))}
   ...
   #   sink()
      ...
      stats <- read.delim(file=paste0("./www/",projeto,"/models/evaluate_ALL_models.txt"),header=F,sep=",",quote="",col.names=c("id","sp","partition","algorithm","AUC","TSS","TSSth", "Kappa","Equal_sens_spec","No_omission","Prevalence","Sensitivity" ))
      #stats
      stats$Sensitivity<-as.numeric(sub(pattern="\"","",stats$Sensitivity))
      stats20 <- stats[order(stats$sp,stats$algorithm,stats$partition),-1]
...
##############################
   ...
   limparResultadosAnteriores()
    ...
    ########################################
    ## INICIO UTILIZAÃÃO FUNÃÃO DISMO.MOD ##
    ########################################
    

    ## RODANDO A FUNÃÃO DISMO.MOD modificada
    futuro = FALSE
    ...
    if (file.exists('www/final/proj_ensemble.tif'))
    {
      rproj <- raster::raster(paste0("www/",projeto,"/final/proj_ensemble.tif"))
    }
    
    output$maparesultadoessemble <- renderLeaflet({
      input$btnModelar
      if (file.exists(paste0("www/",projeto,"/final/ensemble_geral.tif")))
      {
        r <- raster::raster(paste0("www/",projeto,"/final/ensemble_geral.tif"))
    ...
    output$uiarquivosmodelos <- renderUI({
      ...
      lapply(1:length(sort(lista_models)), function(i) {
        tags$div(
          tags$a(href=paste0('/modelagem/',projeto,'/models/',lista_models[i]), paste0(lista_models[i]))
        )
      })
    })
    ...
    output$uifinal <- renderUI({
      ...
      lapply(1:length(sort(lista_modelsfinal)), function(i) {
          tags$a(href=paste0('/modelagem/',projeto,'/final/',lista_modelsfinal[i]), tags$img(src = paste0(projeto,'/final/',lista_modelsfinal[i]), height = "200px"), target="_blank")
    ...
    output$ui <- renderUI({
      ...
      lapply(1:length(order(lista_jpg)), function(i) {
        tags$a(href=paste0('/modelagem/',projeto,'/jpg/',lista_jpg[i]), tags$img(src = paste0(projeto,'/jpg/',lista_jpg[i]), height = "200px"), target="_blank")
      })
    })
    ...
    output$uiscript <- renderUI({
      ...
      lapply(1:length(lista_txt), function(i) {
        tags$div(
          tags$a(href=paste0('/modelagem/',projeto,'/',lista_txt[i]), paste0(lista_txt[i]), target="_blank")
        )
      })
    })
    ...
    output$uiestatistica <- renderUI({
      ...
      lapply(1:length(lista_txt), function(i) {
        tags$div(
          tags$a(href=paste0('/modelagem/',projeto,'/models/',lista_txt[i]), paste0(lista_txt[i]), target="_blank")
        )
      })
    })
    ...
    output$uiarquivosdados <- renderUI({
      ...
      lapply(1:length(lista_csv), function(i) {
        tags$div(
          tags$a(href=paste0('/modelagem/',projeto,'/csv/',lista_csv[i]), paste0(lista_csv[i]), target="_blank")
        )
      })
    })
    ...
    output$uiarquivosensemble <- renderUI({
      ...
      lapply(1:length(sort(lista_final)), function(i) {
        tags$div(
          tags$a(href=paste0('/',projeto,'/final/',lista_final[i]), paste0(lista_final[i]), target="_blank")
        )
      })    
    })
    ...
    output$uiarquivosprojecao <- renderUI({
      ...
      lapply(1:length(sort(lista_proj)), function(i) {
        tags$div(
          tags$a(href=paste0('/',projeto,'/proj/',lista_proj[i]), paste0(lista_proj[i]), target="_blank")
        )
      })    
    })
    ...
    output$uiarquivosprojecaofuturo <- renderUI({
      ...
      lapply(1:length(sort(lista_futuro)), function(i) {
        tags$div(
          tags$a(href=paste0('/',projeto,'/futuro/',lista_futuro[i]), paste0(lista_futuro[i]), target="_blank")
        )
      })    
    })
  ...
  output$mapaabiotico <- renderPlot({
    ...
    isolate({
      ...
      if (length(lista_outros>0))
      {
         ...
         {
           arquivo <- c(arquivo,paste('ex/outros/','bio1.bil',sep=''))
           selecionado = TRUE
         }
      }
...
#    map
	...
	if (input$btnconsultarprojeto>0)
	{
	  ...
	  #  stats2  
	    ...
	    lista_jpg <- list.files(paste0("www/",projeto,"/jpg"),full.names=F,pattern=paste0(".jpg"))
	    lapply(1:length(order(lista_jpg)), function(i) {
	      tags$a(href=paste0('/',projeto,'/jpg/',lista_jpg[i]), tags$img(src = paste0(projeto,'/jpg/',lista_jpg[i]), height = "200px"), target="_blank")
	    })
	  })
	    ...
	    output$uifinal <- renderUI({
	      lista_modelsfinal <- list.files(paste0("www/",projeto,"/final"),full.names=F,pattern=paste0(".png"))
	      lapply(1:length(sort(lista_modelsfinal)), function(i) {
	        tags$a(href=paste0('/',projeto,'/final/',lista_modelsfinal[i]), tags$img(src = paste0(projeto,'/final/',lista_modelsfinal[i]), height = "200px"), target="_blank")
	    ...
	    output$uiarquivosmodelos <- renderUI({
	      ...
	      lapply(1:length(sort(lista_models)), function(i) {
	        tags$div(
	          tags$a(href=paste0('/',projeto,'/models/',lista_models[i]), paste0(lista_models[i]))
	        )
	      })
	    })
	    ...
	    output$uiscript <- renderUI({
	      ...
	      lapply(1:length(lista_txt), function(i) {
	        tags$div(
	          tags$a(href=paste0('/',projeto,'/',lista_txt[i]), paste0(lista_txt[i]), target="_blank")
	        )
	      })
	    })
	    ...
	    output$uiestatistica <- renderUI({
	      ...
	      lapply(1:length(lista_txt), function(i) {
	        tags$div(
	          tags$a(href=paste0('/',projeto,'/models/',lista_txt[i]), paste0(lista_txt[i]), target="_blank")
	        )
	      })
	    })
	    ...
	    output$uiarquivosdados <- renderUI({
	      ...
	      lapply(1:length(lista_csv), function(i) {
	        tags$div(
	          tags$a(href=paste0('/',projeto,'/csv/',lista_csv[i]), paste0(lista_csv[i]), target="_blank")
	        )
	      })
	    })
	...
	if (input$btncriarprojeto>0)
	{
	  ...
	  withProgress(message = '', value = 0, {
	   n <- 6
		...
		incProgress(1/n, detail = paste0("Criando diretório ",projeto))
		Sys.sleep(0.6)
		mkdirs(paste0("www/",projeto,'/final'))
		incProgress(2/n, detail = paste0("Criando diretório ",projeto,"/final"))
		Sys.sleep(0.6)
		mkdirs(paste0("www/",projeto,'/futuro'))
		incProgress(3/n, detail = paste0("Criando diretório ",projeto,"/futuro"))
		Sys.sleep(0.6)
		mkdirs(paste0("www/",projeto,'/jpg'))
		incProgress(4/n, detail = paste0("Criando diretório ",projeto,"/jpg"))
		Sys.sleep(0.6)
		mkdirs(paste0("www/",projeto,'/models'))
		incProgress(5/n, detail = paste0("Criando diretório ",projeto,"/models"))
		Sys.sleep(0.6)
		mkdirs(paste0("www/",projeto,'/proj'))
		incProgress(6/n, detail = paste0("Criando diretório ",projeto,"/proj"))
		Sys.sleep(0.6)
	  }) # progress
